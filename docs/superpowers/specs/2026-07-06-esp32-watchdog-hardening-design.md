...
ottaa `colight_mutex`in seitsemässä kohdassa `xSemaphoreTake(colight_mutex,
...
`susieq_dashboard/src/colight.cpp`: kaikki seitsemän
...
- `colight_notify_handler` (NimBLE-host-taski, 1 kohta): epäonnistuessa
  pudotetaan tämä frame — uusi notifikaatio tulee pian uudestaan, ei
  toiminnallista haittaa.
- `ColightClientCB::onDisconnect` (1 kohta): epäonnistuessa lokitetaan
  varoitus ja palataan — harvinainen polku, ei vaadi erikoiskäsittelyä.
- `colight_task_fn` (3 kohtaa: connected-lipun luku for(;;)-kierroksen
  alussa, stale-clientin siivous, uuden client/chr-julkaisu): epäonnistuessa
  lokitetaan varoitus ja yritetään uudelleen seuraavalla for(;;)-kierroksella
  sen sijaan että jäädään jumiin.
- `colight_read_state()` / `colight_send_command()` (1 kohta kumpikin):
  epäonnistuessa palautetaan `ColightResult` jossa `error = "busy"`,
  `success = false` — sama kuvio kuin `hx711_mutex`in 503 "Sensor busy"
  -vastauksissa. HTTP-pyyntö saa nopean, selkeän vastauksen roikkumisen
  sijaan.
